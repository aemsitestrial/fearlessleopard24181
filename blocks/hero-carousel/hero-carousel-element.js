...
export default async function loadShoelace() {
  if (customElements.get('sl-carousel')) return;
  ...
  const [{ setBasePath }] = await Promise.all([
    ...
    import(`${SHOELACE_CDN}/components/carousel/carousel.js`),
    import(`${SHOELACE_CDN}/components/carousel-item/carousel-item.js`),
  ]);
