...
### Storybook & Shift-Left Testing

This project uses **Storybook** for visual block development, **Vitest** for unit
testing, and **axe-core** (via `@storybook/addon-a11y`) for accessibility
testing. These tools are the shift-left quality gate: every block must pass
them before it is considered done.
...
npm run build-storybook  # build static Storybook (CI)
```

**File conventions** (all required for every block):
- `blocks/<name>/<name>.stories.js` — visual stories; at minimum: Default + edge cases
- `blocks/<name>/<name>.test.js` — Vitest unit tests; cover DOM output, missing fields, empty block

**AEM runtime mocks** live in `test/mocks/` and are shared by both Storybook
and Vitest via module alias. Add stub exports there when new aem.js / scripts.js
functions are needed in tests.

**Shift-left checklist before any PR:**
- [ ] `npm test` exits 0
- [ ] `npm run storybook` — Default story renders without console errors
- [ ] Storybook a11y panel shows **0 violations** on new/changed stories
- [ ] `npm run lint` exits 0

See `.claude/skills/project-specifics/storybook/SKILL.md` for
authoring patterns, mock usage, and troubleshooting.
